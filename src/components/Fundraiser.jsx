import { motion } from 'motion/react';
import { FiArrowRight, FiCheckCircle } from 'react-icons/fi';
import '../styles/fundraiser.css';

const ease = [0.22, 1, 0.36, 1];
const rise = { hidden: { y: 24, opacity: 0 }, show: { y: 0, opacity: 1, transition: { duration: 0.6, ease } } };
const fromRight = { hidden: { opacity: 0, x: 60 }, show: { opacity: 1, x: 0, transition: { duration: 0.8, ease } } };
const pop = { hidden: { opacity: 0, scale: 0.9, y: 16 }, show: { opacity: 1, scale: 1, y: 0, transition: { duration: 0.6, ease } } };


const IMG_MAIN = 'https://res.cloudinary.com/gjpfbvzb/image/upload/v1788112381/WhatsApp_Image_2026-08-29_at_21.37.51_iofqwa.jpg';
const IMG_TOP = 'https://res.cloudinary.com/gjpfbvzb/image/upload/v1787904495/On-the-way-to-Bantry-Day-1_psklnc.jpg';
const IMG_BOTTOM = 'https://res.cloudinary.com/gjpfbvzb/image/upload/v1788179983/IMG_4164-scaled_ru1pwl.webp';
const JUSTGIVING_URL = 'https://res.cloudinary.com/gjpfbvzb/image/upload/v1787861579/Andy-Otacho-23_emkd5z.jpg';

const points = [
  'Cycling the full length of the Leeds–Liverpool Canal - 127 miles (203 km).',
  'Almost entirely off-road, following the canal towpath the whole way.',
  'Raising £5,000 to support 46 children and young people in Migori, Kenya.',
];

export default function Fundraiser() {
  return (
    <section className="fnd">
      <div className="fnd__inner">
        {/* LEFT — content */}
        <motion.div
          className="fnd__content"
          initial="hidden"
          whileInView="show"
          viewport={{ once: false, amount: 0.3 }}
          variants={{ show: { transition: { staggerChildren: 0.12, delayChildren: 0.05 } } }}
        >
          <motion.h2 className="fnd__title" variants={rise}>
            Turning Compassion<br />into Meaningful Impact
          </motion.h2>

          <motion.p className="fnd__lead" variants={rise}>
            This September, we&apos;re cycling the length of the Leeds-Liverpool Canal to raise
            £5,000 for the 46 children and young people we support in Migori, Kenya.
          </motion.p>

          <motion.ul className="fnd__points" variants={{ show: { transition: { staggerChildren: 0.1 } } }}>
            {points.map((p) => (
              <motion.li key={p} className="fnd__point" variants={rise}>
                <FiCheckCircle className="fnd__point-ic" aria-hidden="true" />
                <span>{p}</span>
              </motion.li>
            ))}
          </motion.ul>

          <motion.div variants={rise}>
            <a href={JUSTGIVING_URL} target="_blank" rel="noopener noreferrer" className="fnd__btn">
              Sponsor the Ride <span className="fnd__btn-ic"><FiArrowRight /></span>
            </a>
          </motion.div>
        </motion.div>

        {/* RIGHT — image collage */}
        <motion.div
          className="fnd__media"
          initial="hidden"
          whileInView="show"
          viewport={{ once: false, amount: 0.25 }}
          variants={{ show: { transition: { staggerChildren: 0.15 } } }}
        >
          <motion.div className="fnd__img-main" variants={fromRight}>
            <div className="fnd__img" style={{ backgroundImage: `url(${IMG_MAIN})` }} />
          </motion.div>

          <motion.div className="fnd__img-top" variants={pop}>
            <div className="fnd__img" style={{ backgroundImage: `url(${IMG_TOP})` }} />
          </motion.div>

          <motion.div className="fnd__img-bottom" variants={pop}>
            <div className="fnd__img" style={{ backgroundImage: `url(${IMG_BOTTOM})` }} />
          </motion.div>
        </motion.div>
      </div>
    </section>
  );
}